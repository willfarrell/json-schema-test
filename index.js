...
function jsonSchemaTest(validators, opts) {
  globalThis.describe ??= opts.describe
  var _Promise
  if (opts.async) {
      ...
      files.forEach(function (file) {
        ...
        skipOrOnly(filter, describe)(file.name, function () {
          if (file.test) {
            ...
            var testSets = JSON.parse(readFileSync(testPath))
          }
                ...
                testSet.tests.forEach(function (test) {
                  ...
                  function doTest(validator) {
                    ...
                    function testResults(valid, errors) {
                      ...
                      if (valid) ok(!errors || errors.length == 0)
                      else ok(errors.length > 0)

                      suiteHooks(passed, valid, errors)
                      equal(valid, test.valid)
                    }
